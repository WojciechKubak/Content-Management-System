...
def get_all_translations() -> Response:
    ...
    return make_response({'translations': [translation.to_dict() for translation in translations]}, 200)
...
def update_translation_content(translation_id: int) -> Response:
    ...
    try:
        ...
        updated_translation = api_service.change_translation_content(translation_id, new_content)
...
def update_translation_title(translation_id: int) -> Response:
    ...
    try:
        ...
        updated_translation = api_service.change_translation_title(translation_id, new_title)
        return make_response(updated_translation.to_dict(), 200)
    except (EntityNotFoundError, TranslationNotPendingError, MissingDataError) as e:
        return make_response({'message': str(e)}, 400)
...
def update_translation_status(translation_id: int) -> Response:
    ...
    try:
        ...
        updated_translation = api_service.change_translation_status(translation_id, status, redactor_id)
        return make_response(updated_translation.to_dict(), 200)
    except (EntityNotFoundError, InvalidRedactorIdError,
            InvalidStatusOperationError, TranslationAlreadyReleasedError) as e:
        return make_response({'message': str(e)}, 400)
...
def handle_unrecognized_exception(_: Exception) -> Response:
    ...
    """
    return make_response({'message': 'An unexpected error occurred. Please try again later.'}, 500)
