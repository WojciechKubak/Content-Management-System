...
class TestHandleLanguageEvent:
    ...
    def test_when_exists_on_create(
        self, event_consumer_service: EventConsumerService
    ) -> None:
        language = LanguageFactory()
        dto = LanguageEventDTOFactory(
            id_=language.id, event_type=LanguageEventType.CREATE
        )

        with pytest.raises(EntityAlreadyExistsError) as e:
            event_consumer_service.handle_language_event(dto)

        assert EntityAlreadyExistsError("Language already exists").message == str(
            e.value
        )

    def test_when_not_found_on_update(
        self, event_consumer_service: EventConsumerService
    ) -> None:
    ...
    def test_when_not_found_on_delete(
        self, event_consumer_service: EventConsumerService
    ) -> None:
    ...
    def test_when_unsupported_event_type(
        self, event_consumer_service: EventConsumerService
    ) -> None:
