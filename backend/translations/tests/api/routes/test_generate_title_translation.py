...
class TestGenerateTitleTranslation:

    def test_when_translation_not_found(self, client: Client) -> None:
        response = client.post(
            url_for('translations.generate_title_translation', translation_id=999))
    ...
    def test_when_status_not_pending(self, client: Client) -> None:
        ...
        response = client.post(
            url_for('translations.generate_title_translation', translation_id=translation.id))
        
        assert 400 == response.status_code
        assert { 'message': TranslationNotPendingError().message } == response.json

    def test_when_gpt_service_error(self, client: Client, api_service: ApiService) -> None:
        translation = TranslationFactory()

        with patch.object(api_service.chat_gpt_service, 'get_translation', side_effect=ChatGptServiceError):
            response = client.post(
                url_for('translations.generate_title_translation', translation_id=translation.id))
    ...
    def test_when_translated(self, client: Client) -> None:
        ...
        assert f'TRANSLATED {translation.article.title}' in response.json['title']
