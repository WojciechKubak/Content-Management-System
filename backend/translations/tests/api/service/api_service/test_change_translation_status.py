...
class TestChangeTranslationStatus:
    ...
    def test_when_invalid_status_value(self, api_service: ApiService) -> None:
        ...
        with pytest.raises(InvalidStatusOperationError) as e:
            api_service.change_translation_status(translation.id, f'INVALID {translation.status}', 1)
    ...
    def test_when_status_changed(self, api_service: ApiService) -> None:
        translation = TranslationFactory()
        api_service.change_translation_status(translation.id, StatusType.COMPLETED.value, 1)
        assert sa.session.query(Translation).filter_by(id=translation.id).first().status == StatusType.COMPLETED
