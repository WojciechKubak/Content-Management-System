...
class TestUpdateTag:
    ...
    def test_when_not_found(self, tag_api_service: TagApiService, db_session: Session) -> None:
        tag_dto = {'id': 1, 'name': 'name'}
    ...
    def test_when_updated(self, tag_api_service: TagApiService, db_session: Session) -> None:
        db_session.add(TagEntity(id=1, name='name'))
        db_session.commit()
        tag_dto = {'id': 1, 'name': 'updated_name'}
