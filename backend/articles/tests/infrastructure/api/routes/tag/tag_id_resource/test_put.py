...
class TestIdResourcePut:
    ...
    def test_when_application_error(self, client: Client, base_path: str) -> None:
        with patch(f"{base_path}.{self.resource_path}") as mock_update_tag:
            mock_update_tag.side_effect = ApplicationError()
            response = client.put(url_for("tagidresource", id_=1), json={})
    ...
    def test_when_updated(self, client: Client, base_path: str) -> None:
        ...
        with patch(f"{base_path}.{self.resource_path}") as mock_update_tag:
            ...
            response = client.put(url_for("tagidresource", id_=tag_id), json={})
