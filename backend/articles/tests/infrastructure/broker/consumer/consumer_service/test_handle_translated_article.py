...
class TestHandleTranslatedArticle:
    ...
    def test_when_domain_error(self, consumer_service: ConsumerService) -> None:
        ...
        consumer_service.translation_service.handle_translation_event.side_effect = (
            DomainError("mock_error")
        )
        ...
        consumer_service.translation_service.handle_translation_event.assert_called_once_with(
            "mock_event"
        )

    @pytest.mark.skip("Refactor this test")
    def test_when_handled(self, consumer_service: ConsumerService) -> None:
        mock_dto = Mock(spec=TranslatedArticleDTO)
        mock_dto.to_domain.return_value = "mock_event"
        mock_translation = "mock_translation"
        consumer_service.translation_service.handle_translation_event.return_value = (
            mock_translation
        )
